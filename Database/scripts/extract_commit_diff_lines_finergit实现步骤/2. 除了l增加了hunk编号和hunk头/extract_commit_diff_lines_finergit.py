    ...
    for repo_name in os.listdir(input_dir):
        ...
        if os.path.isdir(repo_path):
            ...
            with open(csv_filename, 'w', newline='', encoding='utf-8') as csvfile:
                ...
                writer.writerow(['commit_id', 'commit_date', 'file_name_mjava', 'file_path', 'repository_name', 'hunk_id', 'hunk_header'])  # 写入表头
...
def process_log_file(log_file_path, writer, repository_name):
    with open(log_file_path, 'r', encoding='utf-8') as log_file:
        ...
        if hunk_headers:  # 如果存在 hunk_headers，写入 CSV
            for hunk_header in hunk_headers:
                writer.writerow([commit_id, commit_date, file_name_mjava, file_path, repository_name, hunk_id, hunk_header])
                hunk_id += 1
